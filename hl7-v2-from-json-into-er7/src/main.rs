...
const USAGE: &str = "\
Convert typed HL7 JSON documents back to HL7 v2.5 ER7 (pipe-delimited).

Usage: hl7_v2_from_json_into_er7 [OPTIONS] [FILE]

Arguments:
  [FILE]  Input file holding one converted-JSON document; \"-\" or omitted reads stdin
...
fn main() -> ExitCode {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "-V" | "--version" => {
                println!("hl7_v2_from_json_into_er7 {}", env!("CARGO_PKG_VERSION"));
                return ExitCode::SUCCESS;
            }
    ...
    let er7_text = match hl7_v2_from_json_into_er7::convert_with_options(&text, options) {
...
fn fail(message: &str) -> ExitCode {
    eprintln!("hl7_v2_from_json_into_er7: error: {message}");
    ExitCode::FAILURE
}
