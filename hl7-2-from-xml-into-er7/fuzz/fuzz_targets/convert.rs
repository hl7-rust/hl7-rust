//! Converting arbitrary XML is total, bounded, and produces valid ER7.
//!
//! Three properties:
//!
...
fuzz_target!(|data: &[u8]| {
    ...
    let Ok(converted) = hl7_2_from_xml_into_er7::convert(text) else {
        return;
    };
    assert!(
        converted.len() <= text.len().saturating_mul(64) + 4096,
        "{} bytes of XML became {} bytes of ER7",
        text.len(),
        converted.len()
    );
