...
use hl7_v2_mllp::{Framer, IoTransport, Tolerance, Transport};
...
fn a_message_survives_the_round_trip_to_the_wire() {
    let frame = hl7_v2_mllp::encode(MESSAGE.as_bytes());
    let payload = hl7_v2_mllp::decode(&frame).unwrap();
...
fn a_stream_that_chops_frames_anywhere_still_yields_whole_messages() {
    let wire = [
        hl7_v2_mllp::encode(MESSAGE.as_bytes()),
        hl7_v2_mllp::encode(b"MSH|^~\\&|A||||1||ACK|2|P|2.5"),
    ]
    .concat();
