...
[`hl7-rust`](https://github.com/hl7-rust/hl7-v2) crate, which re-exports them
behind its optional `derive` feature. Nobody depends on this crate directly.

It exists as a separate crate for one reason: `hl7-rust` advertises exactly
...
into `hl7-rust` that would make its default build pull them in.**

The behavior these macros generate against is specified in the `hl7-rust`
...
tests/derive.rs   Tests, which compile real structs against the real
                  hl7-rust crate — the only way to test a proc macro.
```

`hl7-rust` is a dev-dependency by path, which makes this a dev-dependency
cycle (`hl7-rust` depends on this crate for the `derive` feature). Cargo
...
- **Rust edition 2024.** Dependencies: `proc-macro2`, `quote`, `syn` — and
  nothing else.
- Generated code names everything absolutely (`::hl7::v2::FromHl7Value`,
...
- Before finishing a change, run — in this repo and in `hl7-rust`, because
...
- Attributes that encode HL7 knowledge (segment tables, data types,
  ...
  dictionary's job, in `hl7-rust`. Keep this crate ignorant of HL7.
