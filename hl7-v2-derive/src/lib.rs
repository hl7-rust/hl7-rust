...
fn from_hl7(input: &DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
    for field in named_fields(input)? {
        ...
        reads.push(match mapping(field)? {
            Mapping::Path(path) => quote! {
                #ident: <#ty as ::hl7_2::FromHl7Value>::from_hl7_value(message, #path)?
            },
            Mapping::Nested => quote! {
                #ident: <#ty as ::hl7_2::FromHl7>::from_hl7(message)?
            },
            Mapping::Raw => quote! {
                #ident: ::hl7_2::Raw::new(::core::clone::Clone::clone(message)).into()
            },
            Mapping::None => quote! {
    ...
    Ok(quote! {
        ...
        impl #impl_generics ::hl7_2::FromHl7 for #name #type_generics #where_clause {
            fn from_hl7(message: &::hl7_2::Message) -> ::core::result::Result<Self, ::hl7_2::Error> {
...
fn to_hl7(input: &DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
    for field in named_fields(input)? {
        ...
        match mapping(field)? {
            Mapping::Path(path) => writes.push(quote! {
                <#ty as ::hl7_2::ToHl7Value>::to_hl7_value(&self.#ident, message, #path)?;
            }),
            Mapping::Nested => writes.push(quote! {
                <#ty as ::hl7_2::ToHl7>::to_hl7(&self.#ident, message)?;
            }),
    ...
    Ok(quote! {
        #[automatically_derived]
        impl #impl_generics ::hl7_2::ToHl7 for #name #type_generics #where_clause {
            fn to_hl7(
                &self,
                message: &mut ::hl7_2::Message,
            ) -> ::core::result::Result<(), ::hl7_2::Error> {
                #(#writes)*
