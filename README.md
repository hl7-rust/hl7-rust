...
You do not depend on this crate directly. `hl7-rust` re-exports both macros
...
`hl7-rust` keep exactly one dependency: `syn` and `quote` are compiled only
...
Field types convert through `hl7::v2::FromHl7Value` / `ToHl7Value`:
...
`Error::MissingField`. Implement `hl7::v2::FromHl7Text` for a domain type of
your own and `Option` and `Vec` of it follow.

Writing needs the segments to exist already; build the message with
`hl7::v2::Builder` (whose `encode` method takes a `ToHl7`) or add them with
`Message::append_segment`.

## See also

- [`hl7-rust`](https://github.com/hl7-rust/hl7-v2) — the library these macros
