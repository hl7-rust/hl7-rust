//! HL7 v2.5 data-type knowledge used to name XML elements.
//!
//! In the v2.xml encoding a field element such as `<PID.5>` contains
//! elements named after the field's data type components (`<XPN.1>`,
//! `<XPN.2>`, ...), and a composite component nests its own type
//! (`<XPN.1><FN.1>...</FN.1></XPN.1>`). These tables carry the per-segment
//! field data types and the per-type component data types for the segments
//! and types that appear in common v2.5 messages. Anything not listed here
