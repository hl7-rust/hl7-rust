...
Usage: json_to_hl7_2_5 [OPTIONS] [FILE]
...
fn main() -> ExitCode {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "-V" | "--version" => {
                println!("json_to_hl7_2_5 {}", env!("CARGO_PKG_VERSION"));
                return ExitCode::SUCCESS;
            }
    ...
    let er7_text = match json_to_hl7_2_5::convert_with_options(&text, options) {
...
fn fail(message: &str) -> ExitCode {
    eprintln!("json_to_hl7_2_5: error: {message}");
    ExitCode::FAILURE
}
