...
If `hl7-2-from-er7-into-json` ever stops guaranteeing that the number
...
samples/*.json          Example JSON input files, generated by running the
                       sibling hl7-2-from-er7-into-json crate — see below.
```
...
- **Regenerating samples**: `samples/*.json` are the literal output of
  running the sibling `hl7-2-from-er7-into-json` crate against its own
  ...
  `cargo run --manifest-path ../hl7-2-from-er7-into-json/Cargo.toml -- <input.hl7>`
