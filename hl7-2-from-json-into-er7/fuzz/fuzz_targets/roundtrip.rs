//! ER7 -> JSON -> ER7, over the pair of crates that claims to be each
//! other's inverse.
//!
...
fn trip(er7_text: &str) -> Option<String> {
    let json = hl7_2_from_er7_into_json::convert(er7_text).ok()?;
    hl7_2_from_json_into_er7::convert(&json).ok()
}
