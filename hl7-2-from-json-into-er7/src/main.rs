...
Usage: hl7-2-from-json-into-er7 [OPTIONS] [FILE]
...
fn main() -> ExitCode {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "-V" | "--version" => {
                println!("hl7-2-from-json-into-er7 {}", env!("CARGO_PKG_VERSION"));
                return ExitCode::SUCCESS;
            }
    ...
    let er7_text = match hl7_2_from_json_into_er7::convert_with_options(&text, options) {
...
fn fail(message: &str) -> ExitCode {
    eprintln!("hl7-2-from-json-into-er7: error: {message}");
    ExitCode::FAILURE
}
