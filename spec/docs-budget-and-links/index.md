...
1. **Every tracked markdown document is at most 40 KB** (40,960 bytes).
   ...
   The largest document here is `tasks.md`, which reached 40,917 bytes —
   43 bytes under the 40,960-byte limit — before its oldest entries moved
   verbatim to `tasks-archive.md` on 2026-09-02, so the budget is already
   binding close to the edge, not with room to spare. When a document
