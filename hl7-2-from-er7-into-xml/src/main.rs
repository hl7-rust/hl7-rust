...
Usage: hl7-2-from-er7-into-xml [OPTIONS] [FILE]
...
Options:
  -o, --output <FILE>      Write XML to FILE instead of stdout
      ...
      --dictionary <FILE>  Convert against this JSON dictionary instead of the
                           ...
                           of XSDs with hl7-2-from-xsd-into-json-dictionary
...
fn main() -> ExitCode {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "-V" | "--version" => {
                println!("hl7-2-from-er7-into-xml {}", env!("CARGO_PKG_VERSION"));
                return ExitCode::SUCCESS;
            }
            "--flat" => flat = true,
    ...
    let messages = hl7_2_from_er7_into_xml::split_messages(&text);
    ...
    let options = hl7_2_from_er7_into_xml::Options { flat, schema_shape };
    let mut documents = Vec::with_capacity(messages.len());
    for (i, message) in messages.iter().enumerate() {
        match hl7_2_from_er7_into_xml::convert_with_dictionary(message, &dictionary, options) {
...
fn fail(message: &str) -> ExitCode {
    eprintln!("hl7-2-from-er7-into-xml: error: {message}");
    ExitCode::FAILURE
}
