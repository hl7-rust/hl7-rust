...
whose entire content is `pub use hl7_v2 as v2;` plus its own doc comment.
It exists so a caller can `cargo add hl7` and get `hl7::v2` instead of
depending on `hl7-v2` by name — one module per HL7 standard, so a
...
```
src/lib.rs   The whole crate: `pub use hl7_v2 as v2;`, the `derive` feature
             forwarding to hl7-v2/derive, and the crate-level doc comment
             (which is also the one doctest this crate has).
```

There is no `spec/index.md` here — this crate is too thin to need one.
`hl7-v2`'s spec covers the actual behavior; this crate only re-exports it.

## Working conventions

- **This crate carries no logic of its own.** Parsing, validation,
  dictionaries, all of it lives in `hl7-v2` (and, in time, the other
  ...
  re-exported instead.
- The `derive` feature does nothing but forward to `hl7-v2`'s own `derive`
...
- Implementing HL7 behavior directly in this crate. That's `hl7-v2`'s job
