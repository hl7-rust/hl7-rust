use hl7_v2_from_json_into_er7::{
    convert, convert_with_options, er7::RenderOptions, er7::Terminator,
};

/// The exact converted-JSON document the sibling `hl7-v2-from-er7-into-json`
...
fn parse_returns_a_queryable_message() {
    let json = include_str!("../samples/orm_o01.json");
    let message = hl7_v2_from_json_into_er7::parse(json).unwrap();
