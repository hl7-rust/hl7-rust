...
use hl7_2_soap::response::Outcome;
use hl7_2_soap::{Fault, message, response, wsdl};
...
fn accept(body: &str, allowed: &[String], authorities: &[String]) -> Result<String, Fault> {
    let envelope = hl7_2_soap::parse(body)?;
...
fn the_sender_side_round_trips_an_er7_message() {
    ...
    let envelope = hl7_2_soap::parse(&body).unwrap();
...
fn the_wsdl_describes_the_endpoint_that_served_it() {
    ...
    assert!(hl7_2_soap::xml::parse(&document).is_ok());
}
