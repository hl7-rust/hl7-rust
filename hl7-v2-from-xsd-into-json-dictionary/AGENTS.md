...
src/lib.rs         convert_directory, Options, Error, and the crate
                   documentation. Re-exports hl7-v2-xml-lite-helper as `xml`.
...
One: [`hl7-v2-xml-lite-helper`](../hl7-v2-xml-lite-helper), the small,
dependency-free XML reader shared with `hl7-v2-soap` and
`hl7-v2-from-xml-into-er7`, re-exported from `src/lib.rs` as `xml` so a
...
- Before finishing a change, run:
  ```sh
  cargo test -p hl7-v2-from-xsd-into-json-dictionary
  cargo clippy -p hl7-v2-from-xsd-into-json-dictionary --all-targets -- -D warnings
  cargo fmt --check
  ```
