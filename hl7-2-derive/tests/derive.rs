//! Tests for the macros, run against the real `hl7-2` crate.
//!
...
use hl7_2::{Raw, Version};
use hl7_2_derive::{FromHl7 as DeriveFromHl7, ToHl7 as DeriveToHl7};
...
fn reads_each_annotated_field_from_its_path() {
    let patient: Patient = hl7_2::parse(ADT).unwrap().decode().unwrap();
...
fn nests_structs_and_keeps_the_raw_message() {
    let admission: Admission = hl7_2::parse(ADT).unwrap().decode().unwrap();
...
struct Generic<T>
where
    T: hl7_2::FromHl7Value + hl7_2::ToHl7Value,
{
    #[hl7("PID-1")]
    value: T,
}
...
fn generic_structs_carry_their_bounds_through() {
    let typed: Generic<u32> = hl7_2::parse(ADT).unwrap().decode().unwrap();
